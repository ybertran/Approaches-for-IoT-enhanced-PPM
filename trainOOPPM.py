...
def groupby_pad_num(prefixes, cols, activity_col, numerical_features_col):
    ans_num = groupby_caseID_num(prefixes, cols, numerical_features_col)
    ######ACTIVITY########
    numerical_features = pad_data(ans_num)
    return numerical_features

def groupby_caseID_num(data, cols, numerical_features_col):
    groups = data[cols + [activity_col, numerical_features_col]].groupby(case_id_col, as_index=True)
    ans_num = [torch.tensor(list(y[numerical_features_col])) for _, y in groups]
    return ans_num
...
dt_train_prefixes = dt_train_prefixes.merge(pump_adjustment_counts, on='CaseID', how='left')
...
#prepare inputs
train_cat_cols, test_cat_cols, ce = prepare_inputs(dt_train_prefixes.loc[:, cat_cols], dt_test_prefixes.loc[:, cat_cols])
train_cat_cols, val_cat_cols, _ = prepare_inputs(dt_train_prefixes.loc[:, cat_cols], dt_val_prefixes.loc[:, cat_cols])
...
activity_train, activity_test, activity_val, label_lists_train, label_lists_test, label_lists_val = groupby_pad_all(dt_train_prefixes, dt_test_prefixes, dt_val_prefixes, cols, activity_col)
#OHE all
activity_train_OHE, activity_test_OHE, activity_val_OHE = to_categorical_all(activity_train, activity_test, activity_val, vocab_size[0])
