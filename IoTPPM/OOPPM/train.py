...
#OHE all
activity_train_OHE, activity_test_OHE, activity_val_OHE = datacreator.to_categorical_all(activity_train, activity_test, activity_val, vocab_size[0])
    
preds_all = []
test_y_all = []
score = 0

dataset = torch.utils.data.TensorDataset(activity_train_OHE, numeric_train)
dataset = torch.utils.data.DataLoader(dataset, batch_size=batch_size, drop_last=True)

MODE = "CNN"  # LSTM or CNN
datacreator.check_mode(MODE)
if MODE == "LSTM":
    model = Model(vocab_size, len(numerical_features_col), dropout, lstm_size, num_classes).to(device)
elif MODE == "CNN":
    model = CNNModel(vocab_size, len(numerical_features_col), dropout, num_filters, kernel_size, num_classes).to(device)
...
for epoch in range(epochs):
    ...
    for i, (data_act, data_num) in enumerate(dataset, 0):
        ...
        y_ = model(data_act, data_num).to(device) 
...
with torch.no_grad():
    model.eval()
    print('testing')
    pred = model(activity_test_OHE, numeric_test).squeeze(-1).to(device)
