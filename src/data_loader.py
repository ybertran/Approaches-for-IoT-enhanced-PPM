...
class DataLoader:
    ...
    def load_sensors_dask(self, sensor_path, subset=None):
        """
        Load sensor data using Dask, experimented with it but doesn't really give advantages over pandas because eventually we merge datasets which slows down the whole thing anyways
        """
        ...
        for file, batch_id in batch_files_with_ids:
            ...
            # Use Dask's delayed function to read each CSV file lazily
            df = delayed(pd.read_csv)(file, sep=";", dtype=dtype)
            df = df.assign(batch_id=batch_id)  # Add batch_id column

            dfs.append(df)

        # Concatenate the delayed dataframes using Dask's delayed function
        concatenated = delayed(pd.concat)(dfs, ignore_index=True)

        # Convert the concatenated delayed dataframe to a Dask DataFrame
        dask_df = dd.from_delayed(concatenated)

        # Perform operations on the Dask DataFrame
        dask_df["timestamp"] = dd.to_datetime(dask_df["New Timestamp"])
        dask_df = dask_df.drop(columns=["New Timestamp"])
        dask_df["time_idx"] = dask_df.groupby(["batch_id"]).cumcount()
        # dask_df = dask_df.set_index(["batch_id", "timestamp"])
    ...
    @staticmethod
    def add_lags(df, group_col, lag_columns, num_lags):
        """
